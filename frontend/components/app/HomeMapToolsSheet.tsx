...
type Props = {
  ...
  /** Ana harita şekil çizim modu (web Araç Takımı); seçilince toolbox açılır */
...
export function HomeMapToolsSheet({
  ...
  const [parcelGroupOpen, setParcelGroupOpen] = useState(true);
  const [drawGroupOpen, setDrawGroupOpen] = useState(true);
  const [textGroupOpen, setTextGroupOpen] = useState(true);
  const [markGroupOpen, setMarkGroupOpen] = useState(true);
