...
const formatPriceMaybe = (raw: any, isProMode: boolean): string => {
  if (!isProMode) return '-';
...
export const CombinedScreenshotContainer = React.forwardRef<any, CombinedScreenshotContainerProps>(({
  ...
  const unitPriceText = formatPriceMaybe(unitRaw, isProMode);
  const totalPriceText = formatPriceMaybe(totalRaw, isProMode);
