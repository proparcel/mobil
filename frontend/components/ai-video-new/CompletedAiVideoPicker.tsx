/**
 * AI Video — yalnızca ai_video editor_mode hazır videoları seçici.
 */
...
export function CompletedAiVideoPicker({
  selectedJobId,
  onSelect,
  disabled = false,
  refreshToken = 0,
  ...
  const selectedLabel = useMemo(() => {
    ...
    if (!id) return "AI Video projelerinizden seçin";
    const row = items.find((v) => String(v.job_id) === id);
    if (row) return formatCompletedAiVideoLabel(row);
    return "Seçili video";
  }, [selectedJobId, items]);
  ...
  return (
    <View style={styles.wrap}>
      <Text style={styles.fieldLabel}>Hazır AI Video projeleri</Text>
      <TouchableOpacity
        ...
        accessibilityLabel="Hazır AI Video seç"
      >
        ...
        <Pressable style={styles.backdrop} onPress={() => setOpen(false)}>
          <Pressable style={styles.sheet} onPress={(e) => e.stopPropagation()}>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Tamamlanan AI Video projeleri</Text>
            ...
            {loading ? (
              ...
              <View style={styles.sheetCentered}>
                <Text style={styles.emptyText}>Henüz hazır AI Video yok.</Text>
                  ...
                  return (
                    ...
                    >
                      <Text style={[styles.optionTitle, active && styles.optionTitleActive]} numberOfLines={2}>
                        {formatCompletedAiVideoLabel(item)}
                      </Text>
                      {active ? (
                        <Ionicons
