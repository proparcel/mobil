...
namespace ProParcel.VrParcel.Bridge
{
    ...
    public class VrParcelBridge : MonoBehaviour
    {
        ...
        /** RN terrain viewer — boot sahnesinden güvenli giriş (UnitySendMessage). */
        public void OnLoadTerrainViewer(string jsonOrFileRef)
        {
            Terrain3dAndroidLog.Info("[VrParcelBridge] OnLoadTerrainViewer ref=" +
                (jsonOrFileRef != null && jsonOrFileRef.Length > 80
                    ? jsonOrFileRef.Substring(0, 80) + "..."
                    : jsonOrFileRef));

            var router = ProParcelSceneRouter.Instance ?? FindObjectOfType<ProParcelSceneRouter>();
            if (router != null)
            {
                router.LoadTerrainAndOpen(jsonOrFileRef);
                return;
            }

            TerrainPayloadStore.PendingJson = jsonOrFileRef;
            Terrain3dAndroidLog.Error("[VrParcelBridge] Router henuz yok — payload kuyruga alindi");
        }
