...
import { generateExpertOpinionPdf, type ExpertOpinionPdfOpts } from '../../src/utils/expertOpinionPdf';
...
export default function ExpertRequestReportScreen() {
  ...
  const [photoUpload3Credits, setPhotoUpload3Credits] = useState(20);

  const [pdfGenerating, setPdfGenerating] = useState(false);

  /** Kayıtlı bir ExpertResponse nesnesinden PDF opts üretir */
  const buildPdfOptsFromResponse = useCallback(
    (r: ExpertResponse): ExpertOpinionPdfOpts => {
      const loc = reportData?.location || {};
      return {
        title: 'Uzman Görüşü Raporu',
        location: {
          city: loc.city || '',
        ...
        expertName: r.expert?.fullName || 'Uzman',
        dateStr: r.approvedAt || new Date().toLocaleString('tr-TR'),
        responseText: r.responseText || '',
        surveyAnswers: r.surveyAnswers || {},
        surveyQuestions,
        totalPrice: reportData?.pricing?.totalPrice || null,
        attachmentUrls: (r.attachments || []).map((a: string) => resolveImageUrl(a) || '').filter(Boolean),
      };
    },
    [reportData, surveyQuestions],
  );

  /** Düzenleme formundaki verilerden PDF opts üretir */
  const buildPdfOptsFromForm = useCallback((): ExpertOpinionPdfOpts => {
    const loc = reportData?.location || {};
    return {
      title: 'Uzman Görüşü Raporu',
      location: {
      ...
      expertName: user?.full_name?.trim() || 'Uzman',
      dateStr: new Date().toLocaleString('tr-TR'),
      responseText: responseText || '',
      surveyAnswers: { ...surveyAnswers },
      surveyQuestions,
      totalPrice: reportData?.pricing?.totalPrice || null,
      attachmentUrls: uploadedImages.map((img) => img.uri).filter(Boolean),
    };
  }, [reportData, user, responseText, surveyAnswers, surveyQuestions, uploadedImages]);

  const handlePdfSave = useCallback(
    async (opts: ExpertOpinionPdfOpts) => {
      setPdfGenerating(true);
      try {
        const { pdfUri, filename } = await generateExpertOpinionPdf(opts);
        Alert.alert('PDF Kaydedildi', `${filename} başarıyla oluşturuldu.`);
      } catch (e: any) {
  ...
  const handlePdfShare = useCallback(
    async (opts: ExpertOpinionPdfOpts) => {
      setPdfGenerating(true);
      try {
        const { pdfUri } = await generateExpertOpinionPdf(opts);
        const fileUrl = pdfUri.startsWith('file://') ? pdfUri : `file://${pdfUri}`;
        await Share.open({
          url: fileUrl,
          type: 'application/pdf',
          title: 'Uzman Görüşü PDF Paylaş',
        });
      } catch (e: any) {
        if (e?.message !== 'User did not share') {
          Alert.alert('Hata', e?.message || 'PDF paylaşılamadı');
        }
      } finally {
        setPdfGenerating(false);
      }
  ...
  // photo_upload_3 coin miktarını veritabanından çek
  useEffect(() => {
    getCreditsForEventType('photo_upload_3').then(setPhotoUpload3Credits);
  }, []);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // DFA Snapshot (veritabanından /api/expert-requests/{id}/report-data/ ile gelen)
        const dfaData = reportMeta?.dfaData;
        const dfaAvailable = dfaData?.available && Array.isArray(dfaData?.valuation_steps) && dfaData.valuation_steps.length > 0;
        const pricingFromDfa = dfaAvailable && dfaData?.parcel_values?.unite_price;
        ...
        const hasPricingInAnySource = pricingFromLocalSaved || pricingFromCache || pricingFromBackend || pricingFromDfa;
        const hasGeometryInAnySource = geometryFromLocalSaved || geometryFromCache || geometryFromBackend;
        
        // Pro sorgu gerekli mi?
        // DFA snapshot varsa pro sorguya gerek yok (web ile aynı davranış)
        // DFA yoksa: Fiyat VEYA geometry eksikse yapılmalı
        const needsProQuery = dfaAvailable ? false : (!hasPricingInAnySource || !hasGeometryInAnySource);
        ...
        console.log('[expert-request-report] Pricing kaynakları:', {
          ...
          dfaSnapshot: pricingFromDfa || null,
          hasPricingInAnySource: !!hasPricingInAnySource
        });
        console.log('[expert-request-report] DFA Snapshot durumu:', {
          available: !!dfaAvailable,
          stepsCount: dfaData?.valuation_steps?.length || 0,
          hasPricing: !!pricingFromDfa,
        });
        ...
        console.log('[expert-request-report] Pro sorgu gerekli mi?', {
          needsProQuery,
          dfaAvailable: !!dfaAvailable,
          reason: dfaAvailable ? 'DFA snapshot var, pro sorgu gereksiz' : !hasPricingInAnySource ? 'Fiyat yok' : !hasGeometryInAnySource ? 'Geometry yok' : 'Her ikisi de var'
        });
        
        let proQueryResult: any = null;
        
        // Pro sorgu yapılmama nedenlerini logla
        if (!needsProQuery && dfaAvailable) {
          console.log('[expert-request-report] ✓ DFA Snapshot veritabanından alındı, pro sorgu ATLANACAK');
        } else if (!needsProQuery) {
        ...
        const mergedAreaM2 =
          ...
          null;
        // DFA Snapshot parcel_values (varsa)
        const dfaPv = dfaAvailable ? (dfaData?.parcel_values || {}) : {};
        const pricing = {
          unitPrice: proPv?.unite_price 
            ? parseTurkishPrice(proPv.unite_price) 
            : (dfaPv.unite_price || localSavedQuery?.price_snapshot?.unit_price || cachedData?.pricing?.unitPrice || null),
          totalPrice: proPv?.price_of_tarla 
            ? parseTurkishPrice(proPv.price_of_tarla) 
            : (dfaPv.price_of_tarla || localSavedQuery?.price_snapshot?.total_price || cachedData?.pricing?.totalPrice || null),
          areaM2: dfaPv.alan || mergedAreaM2,
        };
        
        // DFA rows birleştirme (proQueryResult > dfaSnapshot > local savedQuery > cache)
        ...
        } else if (dfaAvailable) {
          // DFA Snapshot veritabanından gelen valuation_steps (pro sorgu yapmadan)
          console.log('[expert-request-report] ✓ DFA Snapshot kullanılıyor, steps:', dfaData.valuation_steps.length);
          const builtRows = buildDfaRowsFromValuationSteps(dfaData.valuation_steps);
          dfaRows = builtRows.map((row: any) => ({
            desc: row.aciklama || row.desc || row.label || '',
            kind: row.tur || row.kind || '',
          }));
        ...
        Alert.alert(
          ...
          isNetworkError 
            ...
            : 'Rapor verisi yüklenemedi. Lütfen tekrar deneyin.',
          [
            { text: 'Geri', onPress: () => router.back(), style: 'cancel' },
  ...
  const isQuestionVisible = useCallback((q: SurveyQuestion): boolean => {
    ...
    if (typeof parentValue === 'object' && parentValue !== null && 'entries' in parentValue) {
      const hasEntries = (parentValue as any).entries && (parentValue as any).entries.length > 0;
    ...
    if (typeof parentValue === 'object' && parentValue !== null && 'ada' in parentValue) {
      const hasAda = (parentValue as any).ada && String((parentValue as any).ada).trim() !== '';
      const hasParsel = (parentValue as any).parsel && String((parentValue as any).parsel).trim() !== '';
  ...
  const validateRequiredSurveyAnswers = useCallback((): string | null => {
    for (const q of surveyQuestions) {
      ...
      if (q.type === 'ada_parsel') {
        const entries = val && (val as any).entries;
    ...
    return null;
  }, [surveyQuestions, surveyAnswers, isQuestionVisible]);

  // Submit response
  const handleSubmitResponse = useCallback(async () => {
    console.log('[expert-request-report] handleSubmitResponse called');
    console.log('[expert-request-report] expertRequestId:', expertRequestId);
    console.log('[expert-request-report] responseText:', responseText);
    console.log('[expert-request-report] surveyAnswers:', JSON.stringify(surveyAnswers, null, 2));
    
    if (!expertRequestId) {
      console.log('[expert-request-report] No expertRequestId, returning');
      return;
    }
    if (!responseText.trim()) {
      console.log('[expert-request-report] No responseText, showing alert');
      Alert.alert('Uyarı', 'Lütfen yanıt metnini giriniz.');
      return;
    }
    const validationError = validateRequiredSurveyAnswers();
    console.log('[expert-request-report] validationError:', validationError);
    if (validationError) {
      Alert.alert('Uyarı', validationError);
      return;
    }

    console.log('[expert-request-report] Starting submission...');
    setSubmitting(true);
    try {
      ...
      console.log('[expert-request-report] Sending respond request with:', {
      ...
      console.log('[expert-request-report] respondResult:', JSON.stringify(respondResult, null, 2));
      ...
      console.log('[expert-request-report] Submit error:', e);
      Alert.alert('Hata', e?.message || 'Yanıt gönderilemedi.');
    } finally {
      console.log('[expert-request-report] Submission finished');
      setSubmitting(false);
    }
  }, [expertRequestId, responseText, surveyAnswers, uploadedImages, router, validateRequiredSurveyAnswers]);
  ...
  const renderRaporTab = () => {
    ...
    const parcelShapeRaw =
      reportData?.parameters_data?.parcel_values?.parcel_shape_type_label ||
      reportData?.parameters_data?.parcel_values?.parcel_shape_type ||
      reportData?.parameters_data?.parcel_values?.edge_measure_data?.parcel_shape_type_label ||
      reportData?.parameters_data?.parcel_values?.edge_measure_data?.parcel_shape_type ||
      null;
    ...
    return (
      <ScrollView contentContainerStyle={styles.tabContent}>
        ...
        <View style={styles.card}>
          ...
          {parcelShapeText ? <Text style={styles.adaParselText}>Parsel Formu: {parcelShapeText}</Text> : null}
        </View>
            ...
            <View style={styles.priceRow}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.priceValue}>
                  {Math.round(reportData!.pricing!.unitPrice || 0).toLocaleString('tr-TR') + ' ₺'}
                </Text>
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.priceLabel}>Toplam</Text>
                <Text style={styles.priceValue}>
                  {Math.round(reportData!.pricing!.totalPrice || 0).toLocaleString('tr-TR') + ' ₺'}
                </Text>
              </View>
            </View>
          </View>
  ...
  const renderYanitTab = () => {
                    ...
                    {(!r.surveyAnswers || Object.keys(r.surveyAnswers).length === 0) ? (
                      ...
                      surveyQuestions.map((q) => {
                        ...
                        const formatPrice = (val: any) => {
                          const num = typeof val === 'number' ? val : parseTurkishPrice(val);
                          return num > 0 ? `${Math.round(num).toLocaleString('tr-TR')} TL` : '-';
                        };
                        const totalPriceLabel = reportData?.pricing?.totalPrice ? Math.round(reportData.pricing.totalPrice).toLocaleString('tr-TR') + ' ₺' : '—';
                        const labelText = (q.label || '').replace('{{toplam_fiyat}}', totalPriceLabel).replace('****', totalPriceLabel);
                        
                        // Fiyat alanlarını tespit et
                        const isPriceField = q.code.includes('price') || q.code.includes('fiyat') || q.type === 'number';
                                ...
                                {answerValue.entries.map((entry: any, entryIdx: number) => (
                                  <View key={entryIdx} style={styles.entryDisplayRow}>
                                    <Text style={styles.surveyValue}>
                                      ...
                                      {entry.fiyat ? ` - ${formatPrice(entry.fiyat)}` : ''}
                                    </Text>
                                  </View>
                                ))}
                              </View>
                        ...
                        } else if (isPriceField && (typeof answerValue === 'number' || typeof answerValue === 'string')) {
                          displayValue = formatPrice(answerValue);
                        ...
                        {r.surveyAnswers?.emsal_ada_parsel?.entries && r.surveyAnswers.emsal_ada_parsel.entries.length > 0 && (
                          <View style={styles.surveySection}>
                            ...
                            {r.surveyAnswers.emsal_ada_parsel.entries.map((entry, entryIdx) => {
                              const alanNum = typeof entry.alan === 'number' ? entry.alan : parseTurkishPrice(entry.alan);
                              const fiyatNum = typeof entry.fiyat === 'number' ? entry.fiyat : parseTurkishPrice(entry.fiyat);
                              return (
                                <View key={entryIdx} style={styles.entryDisplayRow}>
                                  <Text style={styles.surveyValue}>
                                    <Text style={{ fontWeight: '700' }}>{entry.ada}/{entry.parsel}</Text>
                                    {alanNum > 0 ? ` - ${Math.round(alanNum).toLocaleString('tr-TR')} m²` : ''}
                                    {fiyatNum > 0 ? ` - ${Math.round(fiyatNum).toLocaleString('tr-TR')} TL` : ''}
                                  </Text>
                                </View>
                              );
                            })}
              ...
              {r.attachments && r.attachments.length > 0 && (() => {
                ...
                return (
                  <View style={styles.responseSection}>
                    ...
                    {/* PDF Kaydet & Paylaş butonları */}
                    <View style={styles.pdfActionRow}>
                      <TouchableOpacity
                        style={styles.pdfActionBtn}
                        onPress={() => handlePdfSave(buildPdfOptsFromResponse(r))}
                        disabled={pdfGenerating}
                        activeOpacity={0.7}
                      >
                        <Ionicons name="document-text-outline" size={16} color="#3b82f6" />
                        <Text style={styles.pdfActionBtnText}>PDF Kaydet</Text>
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={styles.pdfActionBtn}
                        onPress={() => handlePdfShare(buildPdfOptsFromResponse(r))}
                        disabled={pdfGenerating}
                        activeOpacity={0.7}
                      >
                        <Ionicons name="share-outline" size={16} color="#3b82f6" />
                        <Text style={styles.pdfActionBtnText}>Paylaş</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                )}
    ...
    const myResponse = expertResponses.find((r) => r.expert?.userId === user?.id);
    
    // Eğer yanıt verilmişse, başarı mesajı ve yanıtı göster
    if (myResponse) {
      const formatNum = (n: number) => n?.toLocaleString('tr-TR') || '-';
      const totalPriceLabel = reportData?.pricing?.totalPrice ? Math.round(reportData.pricing.totalPrice).toLocaleString('tr-TR') + ' ₺' : '—';
      ...
      return (
        <ScrollView contentContainerStyle={styles.tabContent}>
          ...
          <View style={styles.successBanner}>
            ...
            <Text style={styles.successText}>Başarılı bir şekilde yanıtladınız</Text>
            {myResponse.approvedAt && (
              <Text style={styles.successDate}>
                {new Date(myResponse.approvedAt).toLocaleDateString('tr-TR', {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric',
                  hour: '2-digit',
          ...
          <View style={styles.card}>
            ...
            {myResponse.responseText && (
              <View style={styles.surveySection}>
                <Text style={styles.surveyLabel}>Açıklamanız</Text>
                <Text style={styles.surveyValue}>{myResponse.responseText}</Text>
              </View>
            )}

            {/* Survey Answers - Dinamik gösterim */}
            {myResponse.surveyAnswers && surveyQuestions.length > 0 && surveyQuestions.map((q) => {
              const answerValue = (myResponse.surveyAnswers as any)?.[q.code];
              if (answerValue === undefined || answerValue === null || answerValue === '') return null;
            ...
            {/* Eklenen Fotoğraflar */}
            {myResponse.attachments && myResponse.attachments.length > 0 && (
              <View style={styles.surveySection}>
                <Text style={styles.surveyLabel}>Fotoğraflar ({myResponse.attachments.length})</Text>
                <View style={[styles.imageGrid, { marginTop: 8 }]}>
                  {myResponse.attachments.map((att: string, idx: number) => {
                    const imgUrl = resolveImageUrl(att);
                    return (
                      <TouchableOpacity
                        key={idx}
                        ...
                        onPress={() => {
                          const resolvedImages = myResponse.attachments.map((a: string) => resolveImageUrl(a) || '');
                          setViewerImages(resolvedImages);
    ...
    // Incoming mode - Response form (henüz yanıt verilmemişse)
    return (
      <ScrollView contentContainerStyle={styles.tabContent}>
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Yanıtınız</Text>
  ...
  return (
    <View style={styles.root}>
      ...
      {/* Submit Button (incoming mode only - eğer yanıt verilmemişse) */}
      {(() => {
        const hasAlreadyResponded = expertResponses.some((r) => r.expert?.userId === user?.id);
        console.log('[expert-request-report] Footer render check - mode:', mode, 'loading:', loading, 'submitting:', submitting, 'hasAlreadyResponded:', hasAlreadyResponded);
        return null;
      })()}
      {mode === 'incoming' && !loading && !expertResponses.some((r) => r.expert?.userId === user?.id) && (
        <View style={[styles.footer, { paddingBottom: insets.bottom + 12 }]}>
          <TouchableOpacity
            style={styles.pdfFooterBtn}
            onPress={() => handlePdfSave(buildPdfOptsFromForm())}
            disabled={pdfGenerating}
            activeOpacity={0.7}
          >
          ...
          <TouchableOpacity
            style={styles.pdfFooterBtn}
            onPress={() => handlePdfShare(buildPdfOptsFromForm())}
            disabled={pdfGenerating}
            activeOpacity={0.7}
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              console.log('[expert-request-report] Submit button pressed!');
...
const styles = StyleSheet.create({
  ...
  imageSlot: {
    width: (WINDOW_WIDTH - 64 - 18) / 4,
    height: (WINDOW_WIDTH - 64 - 18) / 4,
    borderRadius: 6,
    position: 'relative',
  },
  uploadedImage: {
